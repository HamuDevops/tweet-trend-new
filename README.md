...
After that, we set up the backends. Using the details from the DBA’s sheet, we pick which servers will handle the traffic. (I’ll show later how to select the backend servers once the backend set is created, since that part is mentioned later in the sheet.) Then we choose the load balancing policy. Most of the time, we use Weighted Round Robin. This simply means the Load Balancer shares traffic between servers one by one, but gives more traffic to the servers that have a higher weight or capacity.
